...
class ReportsController {
    static async generateMonthlyReport(req, res) {
        try {
            ...
            const totalEstimatedCost = indentData.reduce((sum, row) => sum + (row.total_estimated_cost || 0), 0);
            const totalActualCost = indentData.reduce((sum, row) => sum + (row.actual_cost || 0), 0);
            const completedIndents = indentData.filter((row) => row.status === 'Completed').length;
            // Add summary data
