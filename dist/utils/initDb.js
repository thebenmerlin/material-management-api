...
exports.initializeWithSampleData = void 0;
...
const initializeWithSampleData = async () => {
    try {
        ...
        sites.forEach(site => {
            db_1.DatabaseHelper.executeInsert('INSERT OR IGNORE INTO sites (site_code, site_name, location) VALUES (?, ?, ?)', [site.site_code, site.site_name, site.location]);
        });
        ...
        for (const user of users) {
            ...
            db_1.DatabaseHelper.executeInsert('INSERT OR IGNORE INTO users (username, password_hash, role, site_id, full_name, email) VALUES (?, ?, ?, ?, ?, ?)', [user.username, hashedPassword, user.role, user.site_id, user.full_name, user.email]);
        }
        ...
        materials.forEach(material => {
            db_1.DatabaseHelper.executeInsert('INSERT OR IGNORE INTO materials (material_code, material_name, category, unit, specifications, description) VALUES (?, ?, ?, ?, ?, ?)', [material.material_code, material.material_name, material.category, material.unit, material.specifications, material.description]);
        });
